...
if __name__ == "__main__":
	...
	parser.add_option("-N", "--timing-network", default=False, type="string", help="the filename of a pickled TimingNetwork")
	...
	if opts.verbose: 
		...
		if opts.time: to=triang.time.time()
	detectors = {}
	for arg in args:
		if arg == "H1": #"LHO":
	...
	if opts.verbose and opts.time: print "\t", triang.time.time()-to, "sec"
	...
	if opts.verbose:
		...
		if opts.time: print "\t", triang.time.time()-to, "sec"
	...
	if opts.verbose: 
		...
		if opts.time: to=triang.time.time()
	if opts.prior != "flat":
		ap_map = triang.__ap_map(npix, network, pixarray=None, no_psd=True) # we don't use a psd because this was intended for LHO-LLO networks
	else:
