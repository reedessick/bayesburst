...
2 (\sqrt{2}\sigma_m)^\alpha \left[ \frac{1}{\Gamma{n_fn_p}} \int\mathrm{d} y y^{2n_fn_p + \alpha -1} e^{-y^2} \right] & = C_n 2 (\sqrt{2})^{2\alpha+1} \left(\frac{\sigma_{nm}^2}{\sigma_n\sigma_m}\right)^{2n_fn_p} \sigma^{2\alpha} \left[\left(\frac{1}{\Gamma(n_fn_p)}\right)^2 \int \mathrm{d} y y^{4n_fn_p + 2\alpha -2} e^{-y^2} \right]
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{align}
\sigma_m^\alpha & = C_n \left(\frac{\sigma_{nm}^2}{\sigma_n\sigma_m}\right)^{2n_fn_p} \sigma^{2\alpha} \\
                & = C_n \left(\frac{\sigma_n\sigma_m}{\sigma_n^2+\sigma_m^2}\right)^{2n_fn_p} \left(\frac{\sigma_n^2\sigma_m^2}{\sigma_n^2+\sigma_m^2}\right)^{2\alpha} \\
                & = C_n M_{nm}
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{align}
M_{n>>m} & \sim \left(\frac{\sigma_m}{\sigma_n}\right)^{2n_fn_p} \sigma_m^{2\alpha} \rightarrow 0 \\
M_{nn}   & \sim 2^{-2n_fn_p-\alpha} \sigma_n^{2\alpha} \rightarrow \sigma_n^{2\alpha}
\end{align}
\end{subequations}
...
\sigma_m \delta_{nm} \sigma_n^{-2\alpha} & \sim \\
\sigma_n^{-\alpha} & \sim C_n
\end{align}
\end{subequations}
...
\begin{equation}
\sum_n C_n = \sigma_o^{-\alpha} \sum_n n^{-\alpha}
\end{equation}

which will converge (and therefore be normalizable) iff $\alpha>1$, which is the same condition for the pareto distribution $p(h_{rss}) \propto h_{rss}^{-\alpha}$ that we model.
