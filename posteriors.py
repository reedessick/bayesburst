...
class Posterior(object):
	...
	###
	def set_theta_phi(self, theta_phi=None, coord_sys="E", **kwargs):
		"""
		store theta, phi in coord sys.

		if theta_phi=(theta,phi)==None:
			delegates to self.angPrior.set_theta_phi, which delegates to utils.set_theta_phi
		"""
		if theta_phi!=None: ### set theta,phi to specified values
			self.theta, self.phi = theta_phi
			self.coord_sys = coord_sys
		else: ### default to full-sky healpix decomposition
			if not self.nside:
	...
	def set_A(self, psi=0.0, byhand=False):
		...
		if self.psi==None:
			self.psi = psi
		elif self.psi!=psi: ### make sure we have a consistent psi
			self.psi = psi
			self.B = None
			self.dataB = self.dataB_conj = None

		self.P = self.invP = self.detinvP = None
		self.dpf = False
	...
	def set_dataB(self):
		...
		utils.flatten_and_send(connection, dataB_conj, max_array_size=max_array_size)
	...
	def set_dataB_mp(self, num_proc=1, max_proc=1, max_array_size=100):
		...
		self.dataB_conj = np.empty_like(self.dataB, complex)
                for i in xrange(num_proc):
                        if len(procs): ### if we already have some processes launched
                                ### reap old processes
                                if len(procs) >= max_proc:
					...
					self.dataB_conj[start:end] = utils.recv_and_reshape(con1, shape, max_array_size=max_array_size, dtype=complex)
	...
	#=========================================
	# converstion to and from dominant polarization frame
	#=========================================
	###
	def to_dpf(self, byhand=False):
		""" converts A,B to dominant polarization frame """
		if (self.theta == None) or (self.phi == None):
                        raise ValueError, "set_theta_phi() first"		
		if self.A==None:
			raise ValueError, "set_A first"
		...
		n_pol_eff = self.n_pol
                ...
                n_pix, theta, phi, psi = self.check_theta_phi_psi(self.theta, self.phi, self.psi)
                n_pix, n_freqs, n_pol, A = self.check_A(self.A, n_pix, n_pol_eff)
                n_pix, n_freqs, n_pol, n_ifo, B = self.check_B(self.B, n_pix, n_pol_eff)

		if not self.dpf:
			if self.vecs==None:
				self.A, self.B, self.vecs = self.network.AB_dpf(theta, phi, AB=(self.A, self.B), byhand=byhand)
			else: ### we have vecs
				n_pix, n_freqs, n_pol, vecs = self.check_A(self.vecs, n_pix, n_pol_eff)

				a = np.zeros_like(A, float)
				for i in xrange(n_pol):
	                                for j in xrange(n_pol):
						for k in xrange(n_pol):
	        	                                a[:,:,i,j] += vecs[:,:,k,i] * np.sum( A[:,:,k,:] * vecs[:,:,:,j], axis=-1)
	                        self.A = a

        	                b = np.empty_like(B, complex) ### convert B
                	        for i in xrange(n_pol):
                        	        for j in xrange(n_ifo):
                                	        b[:,:,i,j] = np.sum( vecs[:,:,:,i] * B[:,:,:,j], axis=-1)
	                        self.B = b

			if self.P!=None or self.dataB!=None:
				vecs = self.vecs ### needed for self.P and self.dataB

			if self.P!=None:
				n_pix, n_freqs, n_pol, n_gaus, P = self.check_P(self.P, n_pix, n_pol_eff)
				n_pix, n_freqs, n_pol, n_gaus, invP = self.check_P(self.invP, n_pix, n_pol_eff)
				p = np.zeros_like(P, complex)
				invp = np.zeros_like(invP, complex)
				for g in xrange(n_gaus):
					for i in xrange(n_pol):
                        	                for j in xrange(n_pol):
							for k in xrange(n_pol):
								p[:,:,i,j,g] += vecs[:,:,k,i] * np.sum(P[:,:,k,:,g] * vecs[:,:,:,j], axis=-1)
								invp[:,:,i,j,g] += vecs[:,:,k,i] * np.sum(invP[:,:,k,:,g] * vecs[:,:,:,j], axis=-1)
#								invp[:,:,i,j,g] += vecs[:,:,i,k] * np.sum(invP[:,:,k,:,g] * vecs[:,:,j,:], axis=-1)
                                self.P = p
				self.invP = invp
				### self.detinvP is not affected by the unitary rotation

			if self.dataB!=None:
                                n_pix, n_freqs, n_pol, dataB = self.check_dataB(self.dataB, n_pix, n_pol_eff)
		
				datab = np.empty_like(dataB, complex)	
				for i in xrange(n_pol):	
					datab[:,:,i] = np.sum( vecs[:,:,:,i] * dataB, axis=-1)
				self.dataB = datab
       	                        self.dataB_conj = np.conjugate(datab)

			self.dpf = True

	###
	def from_dpf(self):
		""" converts A, B from dominant polarization frame """
		if (self.theta == None) or (self.phi == None):
                        raise ValueError, "set_theta_phi() first"
		if self.A==None:
			raise ValueError, "set_A first"
		...
		if self.dpf: ### must be in dpf
			a = np.zeros_like(A, float) ### convert A
			for i in xrange(n_pol):
				for j in xrange(n_pol):
					for k in xrange(n_pol):
						a[:,:,i,j] += vecs[:,:,i,k] * np.sum(A[:,:,k,:] * vecs[:,:,j,:], axis=-1)
			self.A = a
			...
			if self.P!=None:
                                ...
                                for g in xrange(n_gaus):
	                                for i in xrange(n_pol):
        	                                for j in xrange(n_pol):
							for k in xrange(n_pol):
	                                                        p[:,:,i,j,g] += vecs[:,:,i,k] * np.sum(P[:,:,k,:,g] * vecs[:,:,j,:], axis=-1)
        	                                                invp[:,:,i,j,g] += vecs[:,:,i,k] * np.sum(invP[:,:,k,:,g] * vecs[:,:,j,:], axis=-1)
#        	                                                invp[:,:,i,j,g] += vecs[:,:,k,i] * np.sum(invP[:,:,k,:,g] * vecs[:,:,:,j], axis=-1)
                                self.P = p
                                self.invP = invp
				### detinvP not affected by unitary rotation
			if self.dataB!=None:
                                n_pix, n_freqs, n_pol, dataB = self.check_dataB(self.dataB, n_pix, n_pol_eff)
...
#	def mle_strain(self, theta, phi, psi=0., n_pol_eff=None, invA_dataB=None, connection=None, max_array_size=100):
	def mle_strain(self, theta, phi, psi=0., invA_dataB=None, connection=None, max_array_size=100):
		"""
...
#	def mle_strain_mp(self, theta, phi, psi, num_proc=1, max_proc=1, max_array_size=100, n_pol_eff=None, invA_dataB=None):
	def mle_strain_mp(self, theta, phi, psi, num_proc=1, max_proc=1, max_array_size=100, invA_dataB=None):
		"""
...
#			h_mle = self.mle_strain(theta, phi, psi, n_pol_eff=n_pol_eff, invA_dataB=(invA,dataB))
			h_mle = self.mle_strain(theta, phi, psi, invA_dataB=(invA,dataB))
...
#		npol_logdf = np.log(df)*n_pol
		...
		for g in xrange(n_gaus):
			...
			ans[:,g,:] += ( np.log( detinvP[:,:,g]) + np.log( detZ ) ).real * 0.5 * self.seglen ### we keep only the real part
#			ans[:,g,:] += ( np.log( detinvP[:,:,g]) - npol_logdf ) / df ### CURRENT NORMALIZATION SCHEME USES UN-NORMALIZED KERNALS
		...
		if diagnostic:
			...
			for g in xrange(n_gaus):
				detZ = self.hPrior.detinvcovariance[:,g]
				det[:,g,:] = ( np.log( detinvP[:,:,g]) + np.log( detZ ) ).real * 0.5 * self.seglen ### tak only the real part
...
#				det[:,g,:] = ( np.log( detinvP[:,:,g]) - npol_logdf ) / df ### CURRENT NORMALIZATION SCHEME USES UN-NORMALIZED KERNALS
		...
		n_pol_eff = n_pol_eff*np.ones((n_pix,),int)
	...
	def log_posterior_elements_mp(self, theta=None, phi=None, psi=0.0,  invP_dataB=None, A_invA=None, num_proc=1, max_proc=1, max_array_size=100, diagnostic=False):
        ...
        ###
        def log_posterior_elements_dfp(self, theta, phi, psi=0.0, n_pol_eff=None, invP_dataB=None, A_invA=None, connection=None, max_array_size=100, diagnostic=False):
                """
                computes the same things as log_posterior_elements, but assumes we work in the dominant polarization frame
		assumes everything handed to it is in the dominant polarization frame
		if things need to be computed, we delegate to self.network for rotations into dominant polarization frame
		P, invP, detinvP will be computed as P=(Adpf+Z), so we should either include a marginalization over rotations of Z or make Z ~ identity*constant

		the optional argument n_pol_eff allows us to project onto a subspace of polarizations. We order polarizations by decreasing eig(A) at each point in the sky.
			if n_pol_eff > self.n_pol, this will raise an error.
                """
                if np.any(self.hPrior.means != 0):
                        raise ValueError, "we only support zero-mean gaussians for now"
			...
			A, B = self.network.AB_dpf(theta, phi, AB=(A,B), no_psd=False)
                        invP = np.empty((n_pix, n_freqs, n_pol, n_pol, n_gaus), complex) ### (A+Z)^{-1}
                        detinvP = np.empty((n_pix, n_freqs, n_gaus), complex)
                        for g in xrange(n_gaus):
                                Z = np.empty_like(A, complex)
...
#                        B = self.network.B(theta, phi, psi, no_psd=False)
				...
				if invP_dataB:
	                                A = self.network.A_dpf(theta, phi, psi, no_psd=False)
                                invA = linalg.inv(A)
...
#                       h_mle = self.mle_strain(theta, phi, psi, n_pol_eff=n_pol_eff, invA_dataB=(invA,dataB))
		...
		### we can truncate things here
		### assume everything is ordered by decreasing eigval(A)
		### this should be how it comes out of utils.network...
		if n_pol_eff==None:
			n_pol_eff = self.n_pol
		elif self.n_pol < n_pol_eff:
			raise ValueError, "n_pol_eff > self.n_pol\nwe can't expand the polarization space"
...
#               npol_logdf = np.log(df)*n_pol
                ...
                for g in xrange(n_gaus):
                        for j in xrange(n_pol_eff):
                                for k in xrange(n_pol_eff): ### even if A is diagonal, invP may not be...
                                        ans[:,g,:] += (dataB_conj[:,:,j] * invP[:,:,j,k,g] * dataB[:,:,k]).real ### we keep only the real part

			### we take determinants each time this is called because it doesn't make sense to store all the possibilities
			Z = self.hPrior.detinvcovaraince[:,:n_pol_eff,:n_pol_eff,g]
                        ans[:,g,:] += ( np.log( np.linalg.det(detinvP[:,:,:n_pol_eff,:n_pol_eff,g]) ) + np.log( np.linalg.det(Z) ) ).real * 0.5 * self.seglen ### we keep only the real part

                ### diagnostic arrays
                if diagnostic:
                        ### mle
                        _mle = np.zeros((n_pix, n_freqs), float)
                        for j in xrange(n_pol_eff): ### assume we are in dpf, so A is diagonal
				_mle += (h_mle_conj[:,:,j] * A[:,:,j,j] * h_mle[:,:,j]).real ### take only the real part
                        for g in xrange(n_gaus):
                                mle[:,g,:] = _mle
                        ...
                        for g in xrange(n_gaus):
                                ...
                                for j in xrange(n_pol_eff): 
                                        ...
                                        for k in xrange(n_pol_eff): ### even if A is diagonal, invP may not be...
                                                ...
                                                cts[:,g,:] -= (diff_conj * Z[:,j,k] * diff).real ### take only real part
                                                for m in xrange(n_pol_eff):
                                                        for n in xrange(n_pol_eff):
                                                                cts[:,g,:] += (diff_conj * Z[:,j,m] * invP[:,:,m,n,g] * Z[:,n,k] * diff).real ### take only real part

                        ### det
                        for g in xrange(n_gaus):
				### we take determinants each time because it is impractical to store/reference all the possibilities
                                Z = self.hPrior.invcovariance[:,:n_pol_eff,:n_pol_eff,g]
                                det[:,g,:] = ( np.log( np.linalg.det(invP[:,:,:n_pol_eff,:n_pol_eff,g]) ) + np.log( np.linalg.det(Z) ) ).real * 0.5 * self.seglen ### tak only the real part
                                                                                                    ### determinant includes detZ because we need to normalize the individual frequencies
                                                                                                    ### see write-up for tentative rationalization
                                                                                                    ### practically, this is needed to prevent determinant-domination for all terms
                                                                                                    ###   without these controlling detZ terms, the determinant can diverge around singular points for A
...
#                               det[:,g,:] = ( np.log( detinvP[:,:,g]) - npol_logdf ) / df ### CURRENT NORMALIZATION SCHEME USES UN-NORMALIZED KERNALS
	...
	###
	def log_posterior_elements_dfp_mp(self, theta=None, phi=None, psi=0.0, n_pol_eff=None, invP_dataB=None, A_invA=None, num_proc=1, max_proc=1, max_array_size=100, diagnostic=False):
		"""
		computes the same things as log_posterior_elements_mp, but assumes we work in the dominant polarization frame
		"""
                ### get positions in the sky    
                if theta==None or phi==None:
                        theta, phi = hp.pix2ang(self.nside, np.arange(self.n_pix))
                ...
                ans = np.empty((n_pix, n_gaus, n_freqs), float) ### array storing all data
                                                                ### (n_pix, n_gaus, n_freqs)
                n_pol_eff_array = np.empty((n_pix,), int)
                ...
                for i in xrange(num_proc):
                        ...
                        if len(procs): ### if we already have some processes launched
                                ...
                                if len(procs) >= max_proc:
                                        ...
                                        shape = np.shape(ans[start:end])
                                        ans[start:end] = utils.recv_and_reshape(con1, shape, max_array_size=max_array_size, dtype=float)
                                        n_pol_eff_array[start:end] = utils.recv_and_reshape(con1, np.shape(n_pol_eff[start:end]), max_array_size=max_array_size, dtype=int)
                                        if diagnostic:
                                                mle[start:end] = utils.recv_and_reshape(con1, shape, max_array_size=max_array_size, dtype=float)
                                                cts[start:end] = utils.recv_and_reshape(con1, shape, max_array_size=max_array_size, dtype=float)
                                                det[start:end] = utils.recv_and_reshape(con1, shape, max_array_size=max_array_size, dtype=float)
                        ...
                        ### launch process
                        con1, con2 = mp.Pipe()
                        p = mp.Process(target=self.log_posterior_elements_dpf, args=(_theta, _phi, _psi, n_pol_eff, _invP_dataB, _A_invA, con2, max_array_size, diagnostic))
                        p.start()
                        con2.close()
                        procs.append( (p, start, end, con1) )
                ...
                if diagnostic:
                        return ans, n_pol_eff_array, (mle, cts, det)
                else:
                        return ans, n_pol_eff_array
	...
	def log_posterior(self, thetas, phis, log_posterior_elements, n_pol_eff, freq_truth, normalize=False, connection=None, max_array_size=100):
		...
		if not isinstance(n_pol_eff, int):
			raise ValueError, "currently only support integer n_pol_eff"

		### sum over frequencies
		df = self.seglen**-1 ### frequency spacing
		summed_log_posterior_elements = np.sum(log_posterior_elements[:,:,freq_truth], axis=2) * 2 * df ### df is included because we're approximating an integral
		                                                                                                ### factor of 2 comes from both sides of the frequency integral

		### sum over gaussians
		log_posterior_weight = utils.sum_logs(summed_log_posterior_elements, coeffs=self.hPrior.get_amplitudes(freq_truth=freq_truth, n_pol_eff=n_pol_eff)) ### amplitudes are the coefficients
...
#			marker.set_alpha(0.5)
	...
	def __call__(self, normalize=True, num_proc=1, max_proc=1, max_array_size=100):
		...
		n_pol_eff = n_pol_eff[0] ### currently only support a single n_pol_eff
